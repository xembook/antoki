...
let onStatusChange;
let nodeFetchTimout = 10000;

async function startSymbolApplication(config,ready,cb1,cb2,cb3,cb4){
...
//		console.log(hashes);
		...
		const ws = await connectWebSocket(targetNode);
		return {
			url:targetNode,
			network:results[1],
			socket:ws
		...
		for(var entry of level.multisigEntries){
			if(entry.multisig.multisigAddresses.length > 0){
				for(var address of entry.multisig.multisigAddresses){
					//childの数だけループを回す
					jstree.push(createChildNode(entry.multisig,address));
				}
			}else{
				jstree.push(createChildNode(entry.multisig,"#"));
			}
		}
	}
//	console.log(jstree);
...
function createChildNode(multisig,parent){

	var child = new Object();
	prettyAddress = sym.Address.createFromEncoded(multisig.accountAddress).pretty();

	child.id = multisig.accountAddress;
	child.text = prettyAddress.slice( 0, 13 ) + "..." +  prettyAddress.slice( -3 );
	child.parent = parent ;
